...
def do_one_batch(model,encoder,revcoder,decoder,encoder_params,decoder_params,batch_de,batch_de_reverse,batch_en,HIDDEN_SIZE,train=True,trainer=None):
    ...
    alpha_init=dy.softmax(dy.concatenate([dy.dot_product(o_init,revcoder_output) for revcoder_output in revcoder_outputs]))
    c_init=attend(revcoder_outputs,alpha_init)
    ...
    for en in batch_en:
        ...
        o_t=s_t.output()
        alpha_t=dy.softmax(dy.concatenate([dy.dot_product(o_t,revcoder_output) for revcoder_output in revcoder_outputs]))
        c_t=attend(revcoder_outputs,alpha_t)
...
def translate_greedy(model,encoder,revcoder,decoder,encoder_params,decoder_params,sentence_de,sentence_de_reverse):
    ...
    o_init=s_init.output()
    alpha_init=dy.softmax(dy.concatenate([dy.dot_product(o_init,revcoder_output) for revcoder_output in revcoder_outputs]))
    c_init=attend_vector(revcoder_outputs,alpha_init)
    ...
    while en!=STOP:
        ...
        o_t=s_t.output()
        alpha_t=dy.softmax(dy.concatenate([dy.dot_product(o_t,revcoder_output) for revcoder_output in revcoder_outputs]))
        c_t=attend_vector(revcoder_outputs,alpha_t)
